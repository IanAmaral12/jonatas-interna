...
as $$
  with settings as (
    select
      p_end_date - p_start_date <= 1 as is_hourly,
...
comment on function public.get_orders_sales_timeline(date, date) is
  'Retorna pedidos não cancelados por hora em períodos de até 48 horas e por dia em períodos maiores, usando o fuso America/Sao_Paulo.';
