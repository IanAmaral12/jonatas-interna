...
as $$
  ...
  general_row as (
    select
      ...
      case
        when traffic.leads > 0 then round((orders.appointments::numeric / traffic.leads) * 100, 2)
      end as conversion_rate,
      case
        when orders.appointments > 0 then round(traffic.leads::numeric / orders.appointments, 2)
    ...
    select
      ...
      case
        when coalesce(traffic.leads, 0) > 0
          then round((coalesce(orders.appointments, 0)::numeric / traffic.leads) * 100, 2)
      end as conversion_rate,
      case
        when coalesce(orders.appointments, 0) > 0
          then round(coalesce(traffic.leads, 0)::numeric / orders.appointments, 2)
      end as lead_to_appointment_ratio,
      case
...
comment on function public.get_cpa_dashboard(date, date) is
  'Retorna mídia, leads, agendamentos não cancelados, faturamento, CPA, conversão, ROAS e ticket médio em BRL.';
