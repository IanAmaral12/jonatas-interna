create or replace function public.get_dashboard_sellers()
returns table (
  id uuid,
  name text
)
...
revoke all on function public.get_dashboard_sellers()
  from public, anon, authenticated;

grant execute on function public.get_dashboard_sellers()
  to authenticated;
...
revoke all on function public.get_orders_sales_timeline(date, date, uuid)
  from public, anon, authenticated;

grant execute on function public.get_orders_sales_timeline(date, date, uuid)
  to authenticated;

comment on function public.get_dashboard_sellers() is
  'Lista os vendedores ativos disponíveis no filtro global da dashboard.';

comment on function public.get_orders_sales_timeline(date, date, uuid) is
  'Retorna séries de pedidos não cancelados, opcionalmente filtradas por vendedor.';
